...
- Cấu trúc source đích là feature-first: `assets`, `components`, `configs`, `features/<feature>/{components,services,types}`, `hooks`, `layouts`, `pages`, `services`, `store`, `utils`, `App.tsx`. Landing scene nằm trong `features/landing/scene`; pages/layouts chỉ composition, không chứa logic Three.js hoặc truy vấn dài.
